...
Every dimension carries `qNullSuppression`, off unless the caller asks
for it with `exclude_null_dimensions=true`. Facts that carry no value for
the grouping field collapse into one row that Qlik renders as `"-"`, and
that row frequently holds a large enough total to win a ranking - on a
live app it held the entire measure total and took rank 1. Leaving it out
is still a statement about the data, so it belongs to the caller rather
than to a default.
