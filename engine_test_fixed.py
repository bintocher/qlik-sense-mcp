...
class QlikEngineTestClient:
    ...
    def generate_comprehensive_report(self, app_id: str) -> Dict[str, Any]:
        """Генерация комплексного отчета о приложении в JSON формате для LLM агентов."""
        logger.info(f"=== ГЕНЕРАЦИЯ КОМПЛЕКСНОГО ОТЧЕТА ДЛЯ {app_id} ===")

        # Собираем все данные
        logger.info("📊 Сбор метаданных приложения...")
        app_metadata = self.analyze_app_metadata(app_id)

        logger.info("📊 Анализ модели данных...")
        data_model = self.analyze_data_model(app_id, include_samples=False)

        logger.info("📊 Анализ использования полей...")
        field_usage = self.analyze_field_usage(app_id)

        logger.info("📊 Анализ мастер-элементов...")
        master_items = self.analyze_master_items(app_id)

        logger.info("📊 Анализ переменных...")
        variables = self.analyze_variables(app_id)
        ...
        logger.info("✅ Комплексный отчет сформирован")
        return report

    def save_comprehensive_report(self, app_id: str, report: Dict[str, Any], filename: str = None) -> str:
        """Сохранение комплексного отчета в JSON файл."""
        if not filename:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            app_name = report.get("app_metadata", {}).get("name", "unknown_app").replace(" ", "_")
            filename = f"qlik_analysis_report_{app_name}_{timestamp}.json"
    ...
    def _print_report_summary(self, report: Dict[str, Any]) -> None:
        """Выводим краткую сводку отчета."""
        logger.info("\n" + "="*80)
        logger.info("📊 СВОДКА КОМПЛЕКСНОГО АНАЛИЗА ПРИЛОЖЕНИЯ")
        logger.info("="*80)

        # Метаданные приложения
        metadata = report.get("app_metadata", {})
        logger.info(f"📱 Приложение: {metadata.get('name', 'N/A')}")
        logger.info(f"📦 Размер: {metadata.get('size_mb', 0)} МБ")
        logger.info(f"📅 Последняя перезагрузка: {metadata.get('last_reload_time', 'N/A')}")
        logger.info(f"✅ Опубликовано: {'Да' if metadata.get('is_published', False) else 'Нет'}")

        # Модель данных
        data_model = report.get("data_model", {})
        logger.info(f"\n📊 Модель данных:")
        logger.info(f"  📁 Таблиц: {data_model.get('total_tables', 0)}")
        logger.info(f"  📋 Полей: {data_model.get('total_fields', 0)}")

        # Использование полей
        summary = report.get("summary", {})
        used_count = summary.get("used_fields", 0)
        unused_count = summary.get("unused_fields", 0)
        total_count = summary.get("total_fields", 0)
    ...
    def _get_field_sample_values(self, app_id: str, field_name: str, table_name: str, max_samples: int = 10) -> List[str]:
        ...
        try:
            ...
            if "error" not in samples_response:
                values = []
                for result in samples_response.get("qResult", []):
                    for value in result.get("qValues", []):
                        if value.get("qIsNumeric", False):
                            values.append(str(value.get("qNumber", "")))
                        else:
                            text_val = value.get("qText", "")
                            if text_val and text_val != "-":
                                values.append(text_val)
                return values[:max_samples]
        except Exception as e:
            logger.warning(f"Не удалось получить примеры для поля {field_name}: {e}")
